...
/// The main profile screen where users view and edit their style profile,
/// including measurements, body type, skin tone, colors, and occasions.
...
class _ProfileScreenState extends State<ProfileScreen>
  ...
  bool             _isSaved       = false;
  bool             _isUploadingImg = false;   // spinner for image upload
  ...
  void initState() {
    ...
    // Load the existing profile from the provider, or fall back to sensible defaults.
    ...
    // Entrance animation: the page fades in and slides up slightly.
    ...
    // Small delay so the animation starts after the first frame is painted.
    Future.delayed(
  ...
  /// Cancelling edit mode discards unsaved changes and restores the last saved profile.
  ...
  /// Adds or removes [name] from the user's favorite colors list.
  ...
  /// Adds or removes [name] from the user's selected occasions list.
  ...
  /// Opens a bottom sheet for the user to choose a photo source (camera / gallery),
  /// then uploads the selected image to Supabase Storage and updates the profile.
  Future<void> _pickImage() async {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              // Only show "Remove Photo" when the user already has one.
    ...
    // Pick image with capped dimensions to keep upload size reasonable.
  ...
  /// Validates inputs, persists the edited profile via [ProfileProvider],
  /// and briefly shows a "Saved" indicator in the app bar.
  ...
  /// Displays an error snack bar and stores the message for the inline banner.
  ...
  /// Displays a success snack bar with a check icon.
  ...
  Widget build(BuildContext context) {
    ...
    // Keep the local edit state in sync with the global profile while not editing,
    // without triggering unnecessary rebuilds if the values haven't changed.
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          // Animated "Saved" badge that appears briefly after a successful save.
          AnimatedSwitcher(
          ...
          child: FadeTransition(
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  // Inline error banner shown below the app bar.
                  ...
                  // Top banner with avatar, name, and quick stats.
                  _AvatarBanner(
                  ...
                  // Summary chips: selected colors count, occasions count, and edit status.
                  _ProfileHighlights(
                    isEditing: _isEditing,
                      ...
                      children: [
                        Row(
                          children: [
                            ...
                            // Badge showing how many colors are currently selected.
                            Container(
                  ...
                  // Save button — only visible while in edit mode.
                  if (_isEditing)
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        ...
                        // Show a spinner while saving; otherwise show the save label.
                        child: _isSaving
                            ? const Row(
                  ...
                  // Admin dashboard shortcut — only visible to admin users.
                  if (isAdmin)
                    SizedBox(
                      width: double.infinity,
                      height: 56,
                  ...
                  // Logout button — signs the user out and navigates to the login screen.
                  SizedBox(
                    width: double.infinity,
                    height: 56,
  ...
  /// Wraps [items] in a horizontal scrollable row with consistent [gap] spacing.
...
// ─────────────────────────────────────────────────────────────────
// Section Card  — a rounded card used to group related profile fields
// ─────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────
// Section Header  — icon + title + subtitle row used at the top of each card
// ─────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────
// Profile Highlights  — three summary chips at the top of the scrollable body
// ─────────────────────────────────────────────────────────────────
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        ...
        const SizedBox(width: AppSpacing.sm),
        // The third chip doubles as an edit-mode indicator.
        Expanded(
          child: _HighlightChip(
...
/// A compact pill-shaped chip that displays an [icon] alongside a [value] label.
...
/// A selectable chip used for single-value options like body type or style personality.
/// Animates its background and border when selected or deselected.
...
// ─────────────────────────────────────────────────────────────────
// Measurement Field  — a labelled numeric input with a unit suffix
// ─────────────────────────────────────────────────────────────────
    ...
    return Column(
      ...
      children: [
        ...
        TextFormField(
          ...
          // Allow up to 3 digits and one decimal place (e.g. 170.5).
...
// ─────────────────────────────────────────────────────────────────
// Color Swatch Button  — circular color picker with selection indicator
// ─────────────────────────────────────────────────────────────────
        ...
        child: Column(
          ...
          children: [
            AnimatedContainer(
              ...
              // Show a check mark when this color is selected.
              child: isSelected
...
// ─────────────────────────────────────────────────────────────────
// Avatar Banner  — gradient card with profile photo, name, and quick stats
// ─────────────────────────────────────────────────────────────────
class _AvatarBanner extends StatelessWidget {
  final UserProfile            profile;
  final bool                   isEditing;
  final bool                   isUploadingImg;
        ...
        children: [
              ...
              children: [
                Container(
                  ...
                  child: isUploadingImg
                      // Spinner shown while the new photo is being uploaded.
                      ? const Center(
                          child: CircularProgressIndicator(
                            strokeWidth: 2.5,
                            color:       Colors.white,
                      ...
                      : profile.imagePath != null
                          // Display the user's photo from Supabase Storage.
                          ? Image.network(
                              profile.imagePath!,
                              fit:    BoxFit.cover,
                              width:  72,
                ...
                // Camera badge overlay — only visible in edit mode while not uploading.
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                // In edit mode, replace the name label with an editable text field.
                if (isEditing)
                  SizedBox(
                    height: 38,
                    child: TextFormField(
