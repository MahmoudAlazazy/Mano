...
/// Admin-only screen for monitoring system health, running API operations,
/// and inspecting provider/snapshot state. Access is gated by [AdminAccessService]
/// or the [forceAdminAccess] bypass flag (for local dev).
class AdminDashboardScreen extends StatefulWidget {
  ...
  /// Skips the normal admin check — useful for local development without a real admin account.
  final bool forceAdminAccess;

  /// Overrides the displayed email when running in forced-admin mode.
...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> {
  ...
  // --- Input controllers for the Create Accounts form ---
  final TextEditingController _countCtrl =
      TextEditingController(text: '100');
  final TextEditingController _threadsCtrl =
      TextEditingController(text: '1');

  // --- UI / operation state flags ---
  bool _stopOnFail = false;
  bool _isAdmin = false;
  bool _checkingAccess = true;        // True while the initial access check is running
  bool _isRefreshingDashboard = false;
  bool _isCreatingAccounts = false;
  bool _isGenerating = false;
  bool _isCheckingClothingSearch = false;
  bool _useImagesField = false;       // Toggles between separate avatar/garments and images[] mode
  ...
  // --- Persistent error list (survives across operations until dismissed) ---
  final List<String> _operationErrors = <String>[];

  // --- Latest statuses from backend health checks ---
  ...
  /// Checks whether the current user qualifies as an admin, then triggers
  /// an initial dashboard refresh if access is granted.
  ...
  /// Fetches API connection status, clothing-image service health, and the
  /// system snapshot in parallel, then reloads all provider data for the
  ...
  Future<void> _refreshDashboard() async {
    ...
    // Kick off all three network calls concurrently.
    ...
    // System snapshot failures are non-fatal; the rest of the dashboard still loads.
    ...
    // Reload all provider data in parallel only when a real user is signed in.
    if (userId != null) {
      await Future.wait([
    ...
    setState(() {
      ...
      // Build a human-readable summary that reflects the snapshot access scope.
  ...
  // ---------------------------------------------------------------------------
  // Clothing Image Search
  // ---------------------------------------------------------------------------

  /// Pings the clothing image service and updates [_clothingImageStatus].
  ...
  /// Opens the gallery for the user to select a single avatar image.
  ...
  /// Opens the gallery for multi-image selection; appends selected paths to [_garmentPaths].
  ...
  /// Validates input, then calls the /create_accounts endpoint with the
  /// configured count, thread count, and stop-on-fail flag.
  ...
  /// Validates that an avatar (and at least one garment, unless in images[] mode)
  /// has been selected, then calls the /generate endpoint.
  ...
  /// Adds [message] to the persistent error list, silently ignoring duplicates.
  ...
  /// Removes [message] from the persistent error list (called when user dismisses a card).
  ...
  /// Builds a single-line error string from an [AdminApiResult], preferring
  /// the explicit error message over the raw body.
  ...
  /// Returns the most useful text representation of an API result:
  /// error message → pretty-printed JSON body → raw body → fallback string.
  ...
  /// Pretty-prints any JSON-serialisable [value]; falls back to toString on error.
  ...
  /// Truncates long strings to a 26-character preview with an ellipsis in the middle.
  ...
  /// Formats a [DateTime] as HH:MM:SS for display in status cards.
  ...
  /// Returns a detail line with result count, sample size, and latency when healthy,
  /// or the error message when unhealthy.
  ...
  /// Resolves a metric value by checking [candidates] keys in order,
  /// returning [fallback] if none are present.
  String _metricValue(
  ...
  /// Safely extracts the `counts` map from the system snapshot.
  ...
  /// Returns a single count value from the snapshot, defaulting to [fallback].
  ...
  /// Returns the top 3 clothing categories as a compact "key:value | ..." string.
  ...
  /// Converts the snapshot's `access_scope` field into a human-readable label.
  ...
  /// Returns the most relevant detail line for the snapshot status card:
  /// errors > warnings > generated_at timestamp.
  ...
  // ---------------------------------------------------------------------------
  // Error Aggregation & Debug Data Builders
  // ---------------------------------------------------------------------------

  /// Collects errors from every provider, the system snapshot, and persistent
  /// operation errors into a deduplicated list for the "Persistent Errors" section.
  List<String> _providerErrors({
    ...
    // Summarise connectivity issues as single actionable messages.
    ...
    return errors.toSet().toList(); // Deduplicate before returning.
  }

  /// Delegates to [AdminAccessService] to produce admin-access diagnostics for the debug console.
  ...
  /// Assembles a snapshot of every provider's current state for the debug console.
  ...
  /// Builds the API trace map: candidate URLs, connection status, clothing image
  /// search state, full connection trace, request history, and last result preview.
  Map<String, dynamic> _buildApiDebug() {
    return <String, dynamic>{
      ...
      'request_history': _api.requestHistory
          .map((attempt) => attempt.toMap())
          .toList(),
      // Body is capped at 400 characters to keep the debug panel readable.
  ...
  /// Builds a collapsible debug console with four sections:
  /// access diagnostics, API trace, provider state, and the raw system snapshot.
  Widget _buildDebugConsole({
    ...
    return Container(
      ...
      child: Theme(
        // Remove the default divider that ExpansionTile adds between header and children.
  ...
  Widget build(BuildContext context) {
    // Watch all providers so the UI rebuilds on any state change.
    ...
    // Data is limited when unauthenticated or when the snapshot is not system-scoped.
    final isDataScopeLimited =
        !auth.isAuthenticated || snapshotScope != 'system_scoped';

    // Count how many providers are actively fetching data.
    ...
    // Show a spinner while the access check is in progress.
    if (_checkingAccess) {
    ...
    // Show a locked screen if the user is not an admin.
    if (!_isAdmin) {
      return Scaffold(
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          // Refresh button — disabled while a refresh is already in progress.
          IconButton(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // Warning banner shown when the snapshot is not system-scoped.
            ...
            // Full global metrics are only available with system-scoped access.
            if (isSystemScoped) ...[
              _StatusCard(
                title: 'Total Users',
              ...
              // Fallback: show the current session's data when scope is limited.
              _StatusCard(
            ...
            if (allErrors.isNotEmpty) ...[
              ...
              const SizedBox(height: AppSpacing.sm),
              ...allErrors.map((error) {
                // Only errors originating from manual operations can be dismissed.
            ...
            // Chips allow individual garment paths to be removed before running /generate.
            Wrap(
            ...
            if (_lastResult != null) ...[
              ...
              // Selectable so the admin can copy the response for further analysis.
              Container(
                width: double.infinity,
                padding: const EdgeInsets.all(AppSpacing.md),
                decoration: BoxDecoration(
...
/// A labelled info card used throughout the dashboard to display key-value
/// status pairs, with an optional secondary detail line.
...
/// A monospace text block used inside the Debug Console to display
/// pretty-printed JSON dumps with a bold title header.
