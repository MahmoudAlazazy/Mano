...
/// Thrown when a Magic Hour API call fails or all keys are exhausted.
...
/// Round-robin pool of Magic Hour API keys. Exhausted keys are skipped until
/// all keys have been marked exhausted, at which point [nextAvailable] throws.
class MagicHourKeyPool {
  ...
  /// Index of the next key to try, advances after each successful pick.
  int _cursor = 0;

  bool get allExhausted => _keys.every((k) => k.exhausted);

  /// Returns the next non-exhausted key in round-robin order.
...
/// Provides access to the Magic Hour API: uploading assets, submitting jobs,
/// polling for results, and downloading output files.
class MagicHourService {
  ...
  /// Returns the best available API key without consuming it from the pool.
  String acquireKey() => _pool.nextAvailable().value;

  void markKeyExhausted(String key) => _pool.markExhausted(key);

  /// Returns true for status codes that indicate a key-level problem
  /// (auth failure, quota, or not found) and warrant trying another key.
  ...
  /// Uploads raw image bytes to Magic Hour's storage and returns the resulting
  /// file path and public URL needed for subsequent API calls.
  Future<MagicHourUploadedAsset> uploadImageBytes({
    ...
    // PUT the raw bytes directly to the pre-signed storage URL.
  ...
  /// Submits an AI clothes-changer job and returns the resulting job ID,
  /// which can be polled via [waitForImageDownloadUrl].
  ...
  /// Fetches the current state of an image project by its job [id].
  ...
  /// Downloads the file at [url] and returns its raw bytes.
  ...
  /// Polls the project status for job [id] until it is complete, then returns
  /// the download URL. Throws if the render fails or [timeout] elapses.
  Future<String> waitForImageDownloadUrl(
    ...
    while (DateTime.now().isBefore(deadline)) {
      ...
      try {
        ...
        // 404 can occur briefly after job creation; keep polling.
        if (e.statusCode == 404) {
  ...
  /// Requests a pre-signed upload URL from the Magic Hour API for a file of
  /// the given [type] and [extension].
  ...
  /// Executes [request] with automatic key rotation. On retryable errors the
  /// current key is marked exhausted and the next key is tried. Non-retryable
  ...
  /// Executes [request] with a specific [key], with no retry or key rotation.
  ...
  /// Returns true for status codes that indicate a key-level problem and
  /// should trigger a retry with a different key.
  ...
  /// Maps a MIME type string to the file extension expected by the Magic Hour API.
  ...
  /// Strips query parameters from a pre-signed upload URL to derive the
  /// clean public URL of the stored file.
...
/// Holds the results of a successful asset upload: the Magic Hour internal
/// file path (used in job requests) and the public download URL.
