...
/// Singleton wrapper around [SupabaseClient] providing all database, storage,
/// and real-time operations for the Outfit Advisor app.
class SupabaseService {
  ...
  /// Parses a metadata value as a list of strings. Accepts a native [List],
  /// a JSON-encoded array string (e.g. `'["red","blue"]'`), or a
  /// comma-separated string. Returns [fallback] if parsing is not possible.
  ...
  /// Casts a raw Supabase query result to a typed list of row maps,
  /// discarding any non-map entries.
  ...
  /// Creates an outfit record and inserts each [clothingItemIds] entry into
  /// `outfit_items` with its position index. Returns the new outfit's ID.
  ...
  /// Fetches an outfit and its associated item rows, returning them merged
  /// under the `"items"` key. Returns null if the outfit is not found.
  ...
  /// Fetches a full cross-table snapshot of the system for admin diagnostics.
  /// Includes row counts, per-user breakdowns, RLS scope detection, and any
  /// table-level errors encountered during the fetch.
  ...
  /// Opens a real-time channel that fires on any change to the user's
  /// `clothing_items` rows. Remember to call [unsubscribe] when done.
  ...
  /// Uploads a local image file to [bucket] under a timestamped path and
